...
Texture2D::Texture2D(const Texture2DDesc& desc, GraphicsEngine* system)
{
	...
	if (FAILED(hr))
	{
		ERROR_3D("Texture2D not created successfully");
	}

	if (desc.type == TextureType::RenderTarget)
	{
		hr = engine->m_d3dDevice->CreateShaderResourceView(this->m_texture.Get(), nullptr, &this->m_shader_res_view);
		if (FAILED(hr))
		{
			ERROR_3D("Texture2D not created successfully");
		}

		hr = engine->m_d3dDevice->CreateRenderTargetView(this->m_texture.Get(), nullptr, &this->m_render_target_view);
		if (FAILED(hr))
		{
			ERROR_3D("Texture2D not created successfully");
		}
	...
	{
		hr = engine->m_d3dDevice->CreateDepthStencilView(this->m_texture.Get(), nullptr, &this->m_depth_stencil_view);
		if (FAILED(hr))
		{
			ERROR_3D("Texture2D not created successfully");
		}
...
Texture2D::Texture2D(const wchar_t* path, GraphicsEngine* system)
{
	...
	if (SUCCEEDED(res))
	{
		res = DirectX::CreateTexture(engine->m_d3dDevice.Get(), mip_chain.GetImages(), mip_chain.GetImageCount(),
			mip_chain.GetMetadata(), reinterpret_cast<ID3D11Resource**>(m_texture.GetAddressOf()));
		...
		desc.Texture2D.MipLevels = static_cast<UINT>(mip_chain.GetMetadata().mipLevels);
		...
		sampler_desc.MaxLOD = static_cast<FLOAT>(mip_chain.GetMetadata().mipLevels);
		...
		res = engine->m_d3dDevice->CreateShaderResourceView(m_texture.Get(), &desc, &m_shader_res_view);
		if (FAILED(res))
		{
			ERROR_3D("Texture2D not created successfully");
		}
