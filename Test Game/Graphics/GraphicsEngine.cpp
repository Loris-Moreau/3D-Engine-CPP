...
GraphicsEngine::GraphicsEngine()
{
	...
	for (UINT driver_type_index = 0; driver_type_index < num_driver_types;)
	{
		res = D3D11CreateDevice(
			nullptr, driver_types[driver_type_index], nullptr, NULL, feature_levels,
...
void GraphicsEngine::compilePrivateShaders()
{
	...
	if (FAILED(D3DCompile(layoutCode, codeLength,"VertexMeshLayoutShader", nullptr, nullptr, "vsmain", "vs_5_0", 0, 0, &blob, &error_blob)))
	{
		...
		ERROR_3D("VertexMeshLayoutShader compiled with errors.")
	}
...
void GraphicsEngine::clearColor(const  SwapChainPtr& swap_chain, const Vec4& color)
{
...
{
	auto dsv = static_cast<ID3D11DepthStencilView*>(swap_chain->getDepthStencilView());

	m_immContext->ClearDepthStencilView(dsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1, 0);
}

void GraphicsEngine::clearColor(const  Texture2DPtr& render_target, const Vec4& color)
{
	if (render_target->m_desc.type != TextureType::RenderTarget)
	{
		return;
	}
...
void GraphicsEngine::setVertexBuffer(const  VertexBufferPtr& buffer)
{
	...
	ui32 offset = 0;
	auto buf = static_cast<ID3D11Buffer*>(buffer->getBuffer());
	m_immContext->IASetVertexBuffers(0, 1, &buf, &stride, &offset);
	m_immContext->IASetInputLayout(static_cast<ID3D11InputLayout*>(buffer->getLayout()));
}

void GraphicsEngine::setIndexBuffer(const  IndexBufferPtr& buffer)
{
	m_immContext->IASetIndexBuffer(static_cast<ID3D11Buffer*>(buffer->getBuffer()), DXGI_FORMAT_R32_UINT, 0);
}

void GraphicsEngine::drawTriangleList(ui32 vertex_count, ui32 start_vertex_index)
{
...
void GraphicsEngine::drawIndexedTriangleList(ui32 index_count, ui32 start_vertex_index, ui32 start_index_location)
{
	m_immContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	m_immContext->DrawIndexed(index_count, start_index_location, start_vertex_index);
}

void GraphicsEngine::drawTriangleStrip(ui32 vertex_count, ui32 start_vertex_index)
{
	m_immContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
	m_immContext->Draw(vertex_count, start_vertex_index);
}

void GraphicsEngine::setViewportSize(ui32 width, ui32 height)
{
	D3D11_VIEWPORT vp = {};
...
{
	m_immContext->VSSetShader(static_cast<ID3D11VertexShader*>(shader->getVertexShader()), nullptr, 0);
	m_immContext->PSSetShader(static_cast<ID3D11PixelShader*>(shader->getPixelShader()), nullptr, 0);
}
...
void GraphicsEngine::setMaterial(const  MaterialPtr& material)
{
	...
	{
		setTexture(&material->m_vec_textures[0], static_cast<UINT>(material->m_vec_textures.size()));
	}
...
void GraphicsEngine::drawMesh(const  MeshPtr& mesh, const std::vector<MaterialPtr>& list_materials)
{
	for (unsigned int m = 0; m < list_materials.size(); m++)
	{
		...
		drawIndexedTriangleList(static_cast<UINT>(mat.num_indices), 0, static_cast<UINT>(mat.start_index));
	}
}
