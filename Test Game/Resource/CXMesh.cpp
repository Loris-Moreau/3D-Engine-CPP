...
CXMesh::CXMesh(const wchar_t* full_path, CXResourceManager* manager) : CXResource(full_path, manager)
{
	...
	m_vertex_buffer = engine->createVertexBuffer({ &list_vertices[0], sizeof(CXVertexMesh), static_cast<UINT>(list_vertices.size()) });
	m_index_buffer = engine->createIndexBuffer({ &list_indices[0], static_cast<UINT>(list_indices.size()) });
}

CXMesh::CXMesh(CXVertexMesh* vertex_list_data, unsigned int vertex_list_size, unsigned int* index_list_data,
	unsigned int index_list_size, CXMaterialSlot* material_slot_list, unsigned int material_slot_list_size,
	CXResourceManager* manager) :
		CXResource(L"", manager)
{
	...
	void* shader_byte_code = nullptr;
	size_t size_shader = 0;
	m_vertex_buffer = engine->createVertexBuffer({ vertex_list_data, sizeof(CXVertexMesh), (UINT)vertex_list_size });
