...
public:
	...
	{
		m_mat[0][0] = (float)scale.m_x;
		m_mat[1][1] = (float)scale.m_y;
		m_mat[2][2] = (float)scale.m_z;
	}
	...
	{
		::memcpy(m_mat, matrix.m_mat, sizeof(m_mat));
	}
	
	Vector3D getXDirection() // Right
	{
		return Vector3D(m_mat[0][0], m_mat[0][1], m_mat[0][2]);
	}

	Vector3D getYDirection() // Up
	{
		return Vector3D(m_mat[1][0], m_mat[1][1], m_mat[1][2]);
	}
	
	Vector3D getZDirection() // Forward
	{
