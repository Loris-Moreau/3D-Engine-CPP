...
Mesh::Mesh(const wchar_t* full_path, ResourceManager* manager) : Resource(full_path, manager)
{
		...
		for (size_t s = 0; s < shapes.size(); s++)
		{
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
			{
				...
				for (unsigned char v = 0; v < num_face_verts; v++)
				{
					...
					m_maxCorner.m_x = (m_maxCorner.m_x < vx) ? vx : m_maxCorner.m_x;
					m_maxCorner.m_y = (m_maxCorner.m_y < vy) ? vy : m_maxCorner.m_y;
					m_maxCorner.m_z = (m_maxCorner.m_x < vz) ? vz : m_maxCorner.m_z;

					m_minCorner.m_x = (m_minCorner.m_x > vx) ? vx : m_minCorner.m_x;
					m_minCorner.m_y = (m_minCorner.m_y > vy) ? vy : m_minCorner.m_y;
					m_minCorner.m_z = (m_minCorner.m_x > vz) ? vz : m_minCorner.m_z;
	...
	m_vertex_buffer = engine->createVertexBuffer({ &list_vertices[0], sizeof(VertexMesh), (UINT)list_vertices.size() });
